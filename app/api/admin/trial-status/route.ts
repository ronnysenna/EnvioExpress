...
export async function GET() {
  try {
    ...
    const tenantsWithTrialStatus = await Promise.all(
      tenants.map(async (tenant) => {
        ...
        if (tenant.subscription) {
          trialInfo = await getTrialInfo(tenant.id);
        }
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        activeTrials: tenantsWithTrialStatus.filter(
          (t) => t.trialInfo?.isOnTrial
        ).length,
        expiringToday: tenantsWithTrialStatus.filter(
          (t) => t.trialInfo?.trialDaysRemaining === 0
        ).length,
        expiredTrials: tenantsWithTrialStatus.filter(
          (t) => t.trialInfo?.hasTrialExpired
        ).length,
        activeSubscriptions: tenantsWithTrialStatus.filter(
