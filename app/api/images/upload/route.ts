...
export async function POST(req: Request) {
  try {
    ...
    // Verificar limites do plano ANTES de fazer upload
    const limitCheck = await checkPlanLimits("upload_image", tenantId);
    ...
    const image = await prisma.image.create({
      data: { url: publicUrl, filename: result.filename || dest, tenantId },
    });

    // Incrementar contador de imagens para este tenant
    await incrementUsage(tenantId, "images", 1);
